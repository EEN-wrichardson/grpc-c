...
grpc_c_context_init (struct grpc_c_method_t *method, int is_client)
{
    ...
    struct grpc_c_method_t *gcm = NULL;
    if (method != NULL) {
        // method* will be freed on call cleanup, so create a new method and copy the passed in method values into
        //  a new allocated struct
        gcm = gpr_malloc(sizeof(struct grpc_c_method_t));
        if (gcm == NULL) {
            gpr_log(GPR_ERROR, "Failed to allocate memory for method %s", method->gcm_name);
            return NULL;
        }

        gcm->gcm_tag = method->gcm_tag;
        gcm->gcm_name = method->gcm_name;
        gcm->gcm_method_id = method->gcm_method_id;
        gcm->gcm_client_streaming = method->gcm_client_streaming;
        gcm->gcm_server_streaming = method->gcm_server_streaming;
        
    }

    context->gcc_method = gcm;
...
grpc_c_context_free (grpc_c_context_t *context)
{
     ...
     */
    if (context->gcc_state == GRPC_C_SERVER_CALLBACK_WAIT || !grpc_c_get_thread_pool()) {
