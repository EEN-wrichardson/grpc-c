              ...
              grpc_channel_args *args)
{
     ...
     */
    if (creds) {
        client->gcc_channel = grpc_secure_channel_create(creds, host, args, NULL);
    } else {
    ...
    if (client->gcc_channel_connectivity_cq == NULL) {
        gpr_log(GPR_ERROR, "Failed to create completion queue for server connect/disconnect notifications");
        grpc_c_client_free(client);
        return NULL;
    }
...
grpc_c_client_free (grpc_c_client_t *client)
{
    ...
    if (grpc_c_get_thread_pool()) {
        ...
        while (client->gcc_running_cb > 0 || client->gcc_wait) {
            gpr_cv_wait(&client->gcc_shutdown_cv, &client->gcc_lock, gpr_inf_future(GPR_CLOCK_REALTIME));
        }
...
{    
    ...
    context->gcc_ops[op_count].data.recv_status_on_client.trailing_metadata = context->gcc_trailing_metadata;
    context->gcc_ops[op_count].data.recv_status_on_client.status = &context->gcc_status;
    context->gcc_ops[op_count].data.recv_status_on_client.status_details = &context->gcc_status_details;
    context->gcc_op_count++;
                       ...
                       grpc_completion_queue *cq)
{
    ...
    grpc_connectivity_state s = grpc_channel_check_connectivity_state(client->gcc_channel, 0);
    ...
    if (s == GRPC_CHANNEL_TRANSIENT_FAILURE || s == GRPC_CHANNEL_SHUTDOWN 
        || (client->gcc_channel_state == GRPC_CHANNEL_READY && s == GRPC_CHANNEL_IDLE)) {
        /*
                 ...
                 gpr_timespec ts)
{
    ...
    while (!shutdown && !timeout) {
        ...
        // ev = grpc_completion_queue_pluck(cq, context, ts, NULL);
            ...
            case GRPC_OP_COMPLETE:
                ...
                */
                if (gcev->gce_type == GRPC_C_EVENT_RPC_INIT) {
                    grpc_c_stream_handler_t *stream_handler = context->gcc_stream;
                    ...
                    */
                    ...
                    context->gcc_method_funcs->gcmf_handler.gcmfh_client(context, context->gcc_tag, ev.success);
                 ...
                 grpc_c_method_data_free_t *output_free)
{
    ...
    for (mdcount = 0; mdcount < mdarray->count; mdcount++) {
        ...
        if (grpc_c_add_metadata(context, (const char *)mdkey, (const char *)mdvalue)) {
    ...
    if (grpc_c_get_thread_pool() && !client->gcc_shutdown) {
        grpc_c_thread_pool_add(grpc_c_get_thread_pool(), gc_run_rpc, context->gcc_cq);
    }
    gpr_mu_lock(&client->gcc_lock);
    client->gcc_running_cb++;
    gpr_mu_unlock(&client->gcc_lock);
                ...
                grpc_c_method_data_free_t *output_free)
{
    ...
    context->gcc_state = GRPC_C_CLIENT_START;
    context->gcm_rpc_exec_type = GRPC_EXEC_SYNCHRONOUS;
    context->gcc_cq = grpc_completion_queue_create_for_pluck(NULL);
    grpc_c_grpc_set_cq_callback(context->gcc_cq, gc_handle_client_event);
                 ...
                 grpc_c_method_data_free_t *output_free)
{
     ...
     * space to hold metadata
     */
    if (grpc_c_add_metadata(context, "client-id", client->gcc_id ? client->gcc_id : "")) {
        grpc_c_context_free(context);
        gpr_log(GPR_ERROR, "Failed to add client-id to metadata");
        return NULL;
    }
                 ...
                 grpc_c_method_data_free_t *output_free)
{
     ...
     */
    context->gcc_call = grpc_channel_create_call(client->gcc_channel, NULL, 0, 
                   context->gcc_cq, 
    ...
    if (e == GRPC_CALL_OK) {
        ...
        gpr_log(GPR_ERROR, "Failed to finish batch operations on async call: %d", e);
        grpc_c_context_free(context);
        return GRPC_C_FAIL;
    }
}
                 ...
                 long timeout)
{
    ...
    if (e == GRPC_CALL_OK) {
        ...
        grpc_c_context_free(context);
        gpr_log(GPR_ERROR, "Failed to finish batch operations on sync call: %d", e);
        rc = GRPC_C_FAIL;
        *output = NULL;
        goto cleanup;
    }
    ...
    ev = grpc_completion_queue_next(context->gcc_cq, tout, NULL);
    ...
    if (ev.type == GRPC_QUEUE_TIMEOUT 
        ...
        *output = NULL;
        ...
        grpc_c_context_free(context);
        gpr_log(GPR_ERROR, "Failed to next sync event");
        rc = GRPC_C_FAIL;
        *output = NULL;
        goto cleanup;
    }
...
grpc_c_client_wait (grpc_c_client_t *client) 
{
    ...
    while (client->gcc_running_cb > 0) {
        gpr_cv_wait(&client->gcc_callback_cv, &mu, gpr_inf_future(GPR_CLOCK_REALTIME));
    }
    gpr_mu_unlock(&mu);
    gpr_mu_destroy(&mu);
...
grpc_c_client_try_connect (grpc_c_client_t *client, long timeout)
{
    ...
    if (grpc_c_grpc_client_try_connect(timeout, gc_client_retry_timeout_cb, client, (void **)&client->gcc_retry_tag)) {
