...
gc_send_initial_metadata_internal (grpc_c_context_t *context, int send)
{
    ...
    if (context->gcc_meta_sent == 0) {
        ...
        context->gcc_ops[context->gcc_op_count].op = GRPC_OP_SEND_INITIAL_METADATA;
        context->gcc_ops[context->gcc_op_count].data.send_initial_metadata.count 
            = context->gcc_initial_metadata->count;

        if (context->gcc_initial_metadata->count > 0) {
            context->gcc_ops[context->gcc_op_count].data.send_initial_metadata.metadata 
        ...
        uint32_t flags UNUSED, long timeout)
{
    ...
    if (context->gcc_payload == NULL) {
        ...
        if (ev.success == 0 && ev.type != GRPC_QUEUE_TIMEOUT) {
            gpr_log(GPR_ERROR, "Failed on read ops");
            context->gcc_read_event.gce_refcount--;
            gpr_mu_unlock(context->gcc_lock);
            return GRPC_C_FAIL;
         ...
         long timeout)
{
    ...
    if (ev.success == 0 || (ev.type != GRPC_OP_COMPLETE && ev.type != GRPC_QUEUE_TIMEOUT)) {
        gpr_log(GPR_ERROR, "Failed on write ops");
        context->gcc_write_event.gce_refcount--;
        gpr_mu_unlock(context->gcc_lock);
        return GRPC_C_FAIL;
    } else if (ev.type == GRPC_QUEUE_TIMEOUT) {
                 ...
                 long timeout)
{
    ...
    if (ev.success == 0 || (ev.type != GRPC_OP_COMPLETE && ev.type != GRPC_QUEUE_TIMEOUT)) {
        gpr_log(GPR_ERROR, "Failed in queue  write done ops");
        context->gcc_write_done_event.gce_refcount--;
        gpr_mu_unlock(context->gcc_lock);
        return GRPC_C_FAIL;
    } else if (ev.type == GRPC_QUEUE_TIMEOUT) {
             ...
             uint32_t flags UNUSED)
{
    ...
    context->gcc_ops[context->gcc_op_count].data.send_status_from_server.status 
        = context->gcc_status;
    context->gcc_ops[context->gcc_op_count].data.send_status_from_server.trailing_metadata_count = 0;
    context->gcc_ops[context->gcc_op_count].data.send_status_from_server.status_details = &status_details;  
