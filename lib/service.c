...
{
    server->gcs_method_funcs[server->gcs_method_count].gcmf_name = gpr_strdup(name);
    server->gcs_method_funcs[server->gcs_method_count].gcmf_handler.gcmfh_server = cb;
    server->gcs_method_funcs[server->gcs_method_count].gcmf_input_packer = input_packer;
    server->gcs_method_funcs[server->gcs_method_count].gcmf_input_unpacker = input_unpacker;
    server->gcs_method_funcs[server->gcs_method_count].gcmf_input_free = input_free;
    server->gcs_method_funcs[server->gcs_method_count].gcmf_output_packer = output_packer;
    server->gcs_method_funcs[server->gcs_method_count].gcmf_output_unpacker = output_unpacker;
    server->gcs_method_funcs[server->gcs_method_count].gcmf_output_free = output_free;
...
gc_register_grpc_method (grpc_c_server_t *server, struct grpc_c_method_t *np) 
{
    ...
    context->gcm_rpc_exec_type = GRPC_EXEC_ASYNCHRONOUS;
...
gc_reregister_method (grpc_c_server_t *server, int method_id)
{
    ...
    for (np = server->gcs_method_list_head.lh_first; np != NULL; np = np->gcm_list.le_next) {
...
gc_prepare_server_callback (grpc_c_context_t *context)
{
    ...
    grpc_c_stream_handler_t *stream_handler = gpr_malloc(sizeof(grpc_c_stream_handler_t));
     ...
     */
    if (context->gcc_data.gccd_server && context->gcc_data.gccd_server->gcs_method_funcs) {
    ...
    gcs_rc_data->op.data.recv_close_on_server.cancelled = &context->gcc_client_cancel;
...
gc_handle_server_complete_op (grpc_c_context_t *context, int success) 
{
    ...
    if (context->gcc_state != GRPC_C_SERVER_CONTEXT_NOOP && context->gcc_call 
        && context->gcc_data.gccd_server->gcs_client_disconnect_cb) {
            grpc_c_grpc_set_disconnect_cb(context->gcc_call, 
                    context->gcc_data.gccd_server->gcs_client_disconnect_cb);
    }
    ...
    if (context->gcc_state == GRPC_C_SERVER_CALLBACK_WAIT) {
        ...
        */
        if (context->gcc_call && client_id != NULL && grpc_c_get_type() > GRPC_THREADS) {
            /*
            ...
            if (grpc_c_grpc_get_client_id(context->gcc_call) == NULL) {
                ...
                */
                if (context->gcc_data.gccd_server->gcs_client_connect_cb) {
                    context->gcc_data.gccd_server->gcs_client_connect_cb(client_id);
                }
        ...
        if (success || context->gcc_call_cancelled) {
            ...
            */
            if (context->gcc_recv_close_event.gce_data) {
                ((gcs_recv_close_data_t *)(context->gcc_recv_close_event.gce_data))->context = NULL;
            }
            grpc_c_context_free(context);
                 ...
                 gpr_timespec ts)
{
    ...
    while (!shutdown && !timeout) {
        ...
        */
        ...
        if (server != NULL && server->gcm_rpc_exec_type == GRPC_EXEC_SYNCHRONOUS) {
            gpr_log(GPR_ERROR, "Server Synchronous executition not supported");
            rc = -1;
            return rc;
        }
        ...
        switch (ev.type) {
                ...
                if (gcev->gce_type == GRPC_C_EVENT_RECV_CLOSE) {
                    ...
                    */
                    ...
                    if (context && context->gcc_read_resolve_cb) {
                        (context->gcc_read_resolve_cb)(context, context->gcc_read_resolve_arg, ev.success);
                    ...
                    */
                    ...
                    if (context && context->gcc_write_resolve_cb) {
                        (context->gcc_write_resolve_cb)(context, context->gcc_write_resolve_arg, ev.success);
                    ...
                    */
                    ...
                    if (context == NULL || context->gcc_data.gccd_server == NULL) {
                        ...
                        */
                        if (gcev->gce_type == GRPC_C_EVENT_WRITE_FINISH || ev.success) {
                            cq = context->gcc_cq;
                        }
                    }
                    ...
                    */
                    ...
                    if (ev.success && state == GRPC_C_SERVER_CALLBACK_WAIT 
                        && !context->gcc_data.gccd_server->gcs_shutdown) {
                            ...
                            if (gc_trace) {
                                gpr_log(GPR_DEBUG, "Incrementing running callbacks on server - %d", 
                                    context->gcc_data.gccd_server->gcs_running_cb);
                            }
            ...
            case GRPC_QUEUE_SHUTDOWN:
                ...
                */
                ...
                if (grpc_c_get_thread_pool() != NULL && server && server->gcs_cq == cq) {
...
grpc_c_server_wait (grpc_c_server_t *server) 
{
    ...
    while (running_cb > 0 || !shutdown) {
        gpr_cv_wait(callback_cv, &mu, gpr_inf_future(GPR_CLOCK_REALTIME));
    }
    gpr_mu_unlock(&mu);
...
grpc_c_methods_alloc (grpc_c_server_t *server, int method_count)
{
    if (server->gcs_method_funcs == NULL) {
        server->gcs_method_funcs = gpr_malloc(method_count * sizeof(grpc_c_method_funcs_t));
        server->gcs_contexts = gpr_malloc(method_count * sizeof(grpc_c_context_t *));
    } else {
        server->gcs_method_funcs = gpr_realloc(server->gcs_method_funcs, 
                           (method_count + server->gcs_method_count) * sizeof(grpc_c_method_funcs_t));
        server->gcs_contexts = gpr_realloc(server->gcs_contexts, 
                       (method_count + server->gcs_method_count) * sizeof(grpc_c_context_t *));
    }
               ...
               grpc_channel_args *args)
{
    ...
    server->gcm_rpc_exec_type = GRPC_EXEC_ASYNCHRONOUS;
...
grpc_c_context_is_call_cancelled (grpc_c_context_t *context)
{
    ...
    gpr_mu_lock(context->gcc_lock);
    if (context->gcm_rpc_exec_type == GRPC_EXEC_SYNCHRONOUS) {
