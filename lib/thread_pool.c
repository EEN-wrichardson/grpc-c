    ...
    for(;;) {
        ...
        */
        if (pool->gctp_shutdown == 0 
            ...
            gpr_cv_wait(&pool->gctp_cv, &pool->gctp_lock, gpr_inf_future(GPR_CLOCK_REALTIME));
            pool->gctp_wait_threads--;
        }
        ...
        */
        if (pool->gctp_shutdown == 0 && pool->gctp_callbacks_head.tqh_first != NULL) {
            struct grpc_c_thread_callback_t *cb = pool->gctp_callbacks_head.tqh_first;
            TAILQ_REMOVE(&pool->gctp_callbacks_head, pool->gctp_callbacks_head.tqh_first, gctc_callbacks);
...
gc_delete_threads (grpc_c_thread_pool_t *pool)
{
    ...
    while (pool->gctp_dead_threads.tqh_first != NULL) {
        ...
        TAILQ_REMOVE(&pool->gctp_dead_threads, pool->gctp_dead_threads.tqh_first, gct_threads);
        gpr_free(thread);
    }
			...
			grpc_c_callback_func_t *func, void *arg)
{
    ...
    if (pool->gctp_wait_threads == 0) {
        ...
        if (pthread_create(&gcthread->p_thread, NULL, gc_thread_func, (void *)gcthread)) {
...
grpc_c_thread_pool_shutdown (grpc_c_thread_pool_t *pool)
{
    ...
    while (pool->gctp_nthreads != 0) {
	    gpr_cv_wait(&pool->gctp_shutdown_cv, &pool->gctp_lock, gpr_inf_future(GPR_CLOCK_REALTIME));
    }
